# The following file is a derivative work of GRML zshrc configuration
...
if zrcautoload compinit ; then
    ...
    # Let compinit check its cache only once a day. 
    if [[ -n ${COMPDUMPFILE}(#qN.mh+24) ]]; then
      _comp_flags=''
    else
      _comp_flags='-C'
    fi
    compinit $_comp_flags -d ${COMPDUMPFILE} "${tmp[@]}" || print 'Notice: no compinit available :('
    unset tmp _comp_flags
else
