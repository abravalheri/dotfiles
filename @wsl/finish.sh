...
__finish_wsl() {
  ...
  echo -e "     - Install Docker Desktop on windows and configure the socket"
  echo -e "       to be exposed via tcp://localhost:2375"

  echo -en "\n  ** $(tput bold)$(tput setaf 4)"
  echo -n      "Enabling \"services\" in WSL"
  echo -e      "$(tput sgr0):"
  echo -e "     - Make sure the scripts '~/.local/bin/start-services.sh'"
  echo -e "       and '~/.local/bin/mount-for-docker.sh'"
  ...
  echo -e "       %sudo ALL=NOPASSWD: /home/<USERNAME>/.local/bin/mount-for-docker.sh"
  echo -e         "$(tput sgr0)"
  echo -e "       References:"
  ...
  echo -e "       - https://nickjanetakis.com/blog/setting-up-docker-for-windows-and-wsl-to-work-flawlessly\n"
